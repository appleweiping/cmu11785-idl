"""Stochastic Gradient Descent optimizer for MyTorch (CMU 11-785 HW1P1).

Operates over ``model.layers`` (a list of :class:`~mytorch.nn.linear.Linear`),
using each layer's ``W``, ``b``, ``dLdW`` and ``dLdb`` attributes.  Supports
classic momentum (``mu``).
"""
